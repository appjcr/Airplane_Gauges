...
static lv_color_t get_hr_zone_color(int32_t hr)
{
    ...
    else if(hr < 25) return lv_palette_main(LV_PALETTE_RED); /* Zone 2 */
...
static void hr_anim_timer_cb(lv_timer_t * timer)
{
    ...
    /* Update needle */
    lv_scale_set_line_needle_value(scale, needle_line, 100, hr_value);
...
void lv_example_scale_10(void)
{
    ...
    lv_obj_set_style_arc_width(scale, 6, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(hr_value_label, &lv_font_montserrat_20, 0);
    ...
    lv_timer_create(hr_anim_timer_cb, 1000, NULL);
}
